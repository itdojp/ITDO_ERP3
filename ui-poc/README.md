...
環境変数 `PM_PORT` (ホスト側の公開ポート、既定 3001) と `UI_PORT` (既定 4000) を指定するとポートを変更できます。必要に応じて `PM_CONTAINER_PORT` でコンテナ内ポートも調整できますが、通常は既定の 3001 のままで問題ありません。PoC API サービスは `/app/state/pm-poc-state.json`（`PM_STATE_FILE` で変更可）に状態を永続化するため、Podman を再起動しても最新状態が復元されます。状態をリセットしたい場合は `scripts/reset_pm_state.sh` を実行してください。
...
- Podman 連携時: `scripts/run_podman_ui_poc.sh` でスタックを起動し、Real API モード (`API live` バッジ) とモックモード (`Mock data`) が自動で切り替わることを確認。
  ...
  - `POST /api/v1/projects` や `POST /api/v1/timesheets` で新しいデータを投入し、`/metrics/summary` や `/events/recent` でイベント状況を確認できます。
...
- Podman を起動して API 連携（`API live` バッジ）を確認したい場合は `cd ui-poc && npm run test:e2e:live` を利用してください。
